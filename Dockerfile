...
RUN yum check-update -y ; \
    ...
    yum install -y --setopt=tsflags=nodocs sudo which wget file && \
...
RUN /bin/echo -e "    wget --no-check-certificate http://crossover.codeweavers.com/redirect/crossover.bin -O /tmp/install-crossover.bin && chmod +x /tmp/install-crossover.bin && /tmp/install-crossover.bin --i-agree-to-all-licenses --destination ${INSTALLDIR} --noreadme --noprompt --nooptions && echo -e 'install Completed\n' && rm -f /tmp/install-crossover.bin" >> ${HOME}/.vnc/xstartup
